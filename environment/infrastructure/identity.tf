...
resource "azurerm_role_assignment" "ccm" {
  ...
  depends_on = [azurerm_role_definition.ccm, azurerm_user_assigned_identity.vcluster_node]
}
...
resource "azurerm_role_assignment" "csi" {
  for_each = local.csi_enabled ? { enabled = true } : {}
  ...
  depends_on = [azurerm_role_definition.csi, azurerm_user_assigned_identity.vcluster_node]
}
